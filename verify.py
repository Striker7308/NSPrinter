...
def check_store_info(store, receipt, jiuxun):
    return {
        '刷卡门店': True if receipt["门店"] == store[jiuxun.get('销方公司名称', '')]['name'] else "小票门店 "+receipt["门店"]+' != 九讯云门店 '+store[jiuxun['销方公司名称']]['name'],
...
def check_customer_info(id, receipt, jiuxun):
    return {
        ...
        '联系人': True if id['name'] == jiuxun['联系人'] == jiuxun['发票抬头'] else '身份证照片 '+id['name']+' != 九讯云联系人 '+jiuxun['联系人']
    }
...
def check_merchandise_info(phone, receipt, jiuxun):
    ...
    return {
        ...
        'SN': True if phone['SN'] == jiuxun['SN'] else '手机屏显SN '+phone['SN']+' != 九讯云上SN '+jiuxun['SN'],
        # 'IMEI1有误': True if phone['IMEI1'] == receipt['phone_info']['IMEI1'] else "手机显示IMEI1-"+phone['SN']+' ?= '+'小票显示IMEI1-'+receipt['phone_info']['SN'],
        'IMEI1': True if jiuxun['分类'] != '智能手机' or phone['IMEI1'] == jiuxun['序列号'] else '手机屏显IMEI1 '+phone['IMEI1']+' != 九讯云上IMEI1 '+jiuxun['序列号'],
